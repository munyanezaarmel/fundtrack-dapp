    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 py-12 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Manage your projects and track your investments</p>
        </div>
    ...
    <div className="border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-shadow">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div>
          ...
          <div className="text-lg font-bold text-blue-600">{formatEther(project.fundsRaised)} ETH</div>
        </div>
        <div>
          <div className="text-xs text-gray-600 mb-1">Target</div>
          <div className="text-lg font-bold text-gray-900">{formatEther(project.targetAmount)} ETH</div>
        </div>
        <div>
          <div className="text-xs text-gray-600 mb-1">Released</div>
          <div className="text-lg font-bold text-purple-600">{formatEther(project.fundsReleased)} ETH</div>
        </div>
        <div>
            ...
            <div key={idx} className="flex items-center">
              <div
                className={`w-8 h-8 rounded-full mr-3 flex items-center justify-center ${
                  milestone.completed ? "bg-green-500 text-white" : "bg-gray-200 text-gray-500"
                }`}
              >
                {milestone.completed ? "✓" : idx + 1}
              </div>
              <div className="flex-1">
                <div className={`font-medium ${milestone.completed ? "text-gray-900" : "text-gray-500"}`}>
                  {milestone.title}
                </div>
                <div className="text-xs text-gray-500">
                  {milestone.percentage}% of funds
                  {milestone.completed && <span className="ml-2 text-green-600">✓ Completed</span>}
                </div>
              </div>
            </div>
          ))}
